...
<div class="container p-5 bg-dark text-white rounded shadow mt-4">
    ...
    <form action="Pages/Student/Blog" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="Tomtat" id="Tomtat" class="form-control ckeditor" rows="3"></textarea>
            @error('Tomtat')
                <div class="text-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="Noidung" id="Noidung" class="form-control ckeditor" rows="6"></textarea>
            @error('Noidung')
                <div class="text-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
...
<div class="container mt-5 p-4 bg-light rounded shadow">
        ...
        <table class="table table-hover table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $blg->title }}</td>
                    <td>
                        <form action="Pages/Student/getUpdateBlog/{{ $blg->id_blog }}" method="get">
                            @csrf
                            <button type="submit" class="btn btn-primary btn-sm">Sửa</button>
                        </form>
                    </td>
                    <td>
                        <form action="Pages/Student/delBlog/{{ $blg->id_blog }}" method="get">
                            @csrf
    ...
    <div class="d-flex justify-content-center mt-3">
        {{ $BL_St->links('pagination::bootstrap-4') }}
    </div>
</div>
