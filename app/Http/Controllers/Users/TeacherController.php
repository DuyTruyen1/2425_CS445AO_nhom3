...
class TeacherController extends Controller
{
    ...
    public function updateBlog(PostBlogRequest $request, $id_blog)
    {
        ...
        return view('Pages.Teacher.Blog', ['category' => $category, 'BL_Tr' => $BL_Tr])
    ...
    public function postUpdate(UpdateTeacherRequest $request, $id)
    {
        $kcheck = [];
        $skill = skill::all(); // Lấy tất cả các kỹ năng
        $skill_id = $request->skill_id;

        // Kiểm tra nếu không có id thì trả về lỗi hoặc tạo mới
        $teacher = teacher::find($id); // Tìm giáo viên theo id

        // Lấy category thứ 2 từ bảng category
        $category = category::all()[1];

        if ($teacher == null) {
            // Nếu không tìm thấy giáo viên, tạo mới
            ...
            if ($teacher2->save()) {
                if ($skill_id) {
                    FK_Skill::where('teacher_id', $teacher2->id)->delete();
                    foreach ($skill_id as $sk) {
                        FK_Skill::create(['skill_id' => $sk, 'teacher_id' => $teacher2->id]);
                    }
                } else {
                    FK_Skill::where('teacher_id', $teacher2->id)->delete();
                }
            ...
            // Lấy thông tin kỹ năng đã lưu
            $skill_all = Fk_Skill::select('skill.name')
                ->join('skill', 'skill.id', '=', 'fk_skill.skill_id')
                ->where('fk_skill.teacher_id', $teacher2->id)
                ->get()->toArray();
            foreach ($skill_all as $k) {
                array_push($kcheck, $k['name']);
            }

            return redirect()->route('teacher.profile', ['id' => $teacher2->id])
                ->with([
                    'category' => $category,
                    'skill' => $skill,
                    'skillcheck' => $kcheck,
                    'success' => 'Bạn cập nhật thông tin thành công!'
                ]);
        } else {
            // Nếu tìm thấy giáo viên, cập nhật thông tin
            ...
            if ($teacher->save()) {
                if ($skill_id) {
                    FK_Skill::where('teacher_id', $teacher->id)->delete();
                    foreach ($skill_id as $sk) {
                        FK_Skill::create(['skill_id' => $sk, 'teacher_id' => $teacher->id]);
                    }
                } else {
                    FK_Skill::where('teacher_id', $teacher->id)->delete();
                }
            ...
            $skill_all = Fk_Skill::select('skill.name')
                ->join('skill', 'skill.id', '=', 'fk_skill.skill_id')
                ->where('fk_skill.teacher_id', $teacher->id)
                ->get()->toArray();
            foreach ($skill_all as $k) {
                array_push($kcheck, $k['name']);
            }
